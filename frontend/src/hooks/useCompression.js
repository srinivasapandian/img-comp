...
import { isCompressFile, traverseEntry, formatBytes } from '../utils/fileUtils'
import { startCompression, subscribeProgress, downloadZip, cancelSession } from '../utils/api'
...
export const useCompression = () => {
  ...
  const addFiles = useCallback(
    async (dropItems) => {
      ...
      for (const item of dropItems) {
        // DataTransferItem with webkitGetAsEntry → folder support
        if (item && typeof item.webkitGetAsEntry === 'function') {
          ...
          if (entry) {
            ...
            traversed.forEach(({ file, relativePath }) => {
              if (isCompressFile(file)) collected.push({ file, relativePath, id: genId() })
            })
            continue
          }
        }

        // Plain File object (file input or fallback)
        const file = item instanceof File ? item : item.getAsFile?.()
        if (file && isCompressFile(file)) {
      ...
      if (collected.length === 0) {
        addToast('warning', 'No supported images found (JPG, PNG, WEBP)')
        return
      }
  ...
  const compress = useCallback(async () => {
    ...
    try {
      const { session_id } = await startCompression(
        files.map((f) => f.file),
      ...
      unsubRef.current = subscribeProgress(
        ...
        (data) => {
          ...
          if (data.status === 'completed') {
            setPhase('done')
            addToast('success', `Done! Saved ${formatBytes(data.stats?.saved || 0)} across ${data.processed} file(s)`)
          } else if (data.status === 'failed') {
            setPhase('error')
            addToast('error', 'Compression failed — check logs')
          }
        },
        () => {
  ...
  const cancel = useCallback(async () => {
    ...
    setSession(null)
    setPhase('ready')
    addToast('info', 'Compression cancelled')
  }, [addToast])
  ...
  const download = useCallback(async () => {
    ...
    if (!id) return
    try {
      await downloadZip(id, 'compressed')
